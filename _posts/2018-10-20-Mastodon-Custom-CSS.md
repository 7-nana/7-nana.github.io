...
SCSSによるテーマ追加が思うようにできなかった方や、Mastodonのホスティングサービスを利用している方にはとても便利な機能です。

## カスタムCSSの入力欄

カスタムCSSは、管理画面の「管理」＜「サイト設定」の下部にあります。すぐ上に「カスタム詳細説明」や「カスタム利用規約」の入力欄があるので、誤って上書いてしまわないように注意しましょう。
...
残念なことに、管理画面のbodyタグには固有のクラス名が割り振られていないため、CSSをカスタマイズしにくい状態です。できないことはありませんが、ダークテーマでもライトテーマでも問題なく視認できるか、確認しながら作業する必要があります。

管理画面もカスタマイズしたい場合は、今のところSCSSによるテーマ追加の方法をとった方が良いと思います。

## 「ハイコントラスト」テーマをカスタマイズする

私は「Mastodon」と「Mastodon（ライト）」はあまり触らずに置いておき、「ハイコントラスト」を大幅にカスタマイズすることにしました。

ただしテーマ名は変えられないので、ユーザーが困惑しないよう、カスタマイズ後も「ハイコントラスト（背景と文字がはっきり区別できる状態）であること」は維持します。また、前述の通り管理画面のCSSを触れないので、テーマの明るいところ・暗いところも維持します。
...
興味がある方はコピペして使ってみてください。このCSSはMastodonのライセンス同様、どなたでも自由に使用することができます。
...
ここは「何色で塗りつぶす（fill）か」を指定しています。シングルクォーテーションで囲っている部分、先頭の「%23」はエンコードした「#」なので、そのうしろの文字列が16進数のカラーコードというわけです。
...
以上のように、Mastodon v2.5.0からは、ホスティングサービスを利用しているインスタンスでも見た目の個性が出せるようになりました。こういったカスタマイズは、ブログのカスタマイズと同じようにとても楽しいものですし、ワクワクしますね。

あとは、Web UI左下や管理画面左上のMastodon画像をオリジナルのものに変更できて、自動的にfaviconにも反映されたら良いのにな〜、なんて思ってます（他力本願）。
