...
残念なことに、管理画面のbodyタグにはテーマ固有のクラス名が無く、カスタマイズしにくい状態です。できないことはありませんが、全てのテーマの管理画面を変更してしまうため、ダークテーマでもライトテーマでも問題なく視認できるか、確認しながら作業する必要があります。
...
ただしテーマ名は変えられないので、ユーザーが困惑しないよう、カスタマイズ後もテーマ名の通り「ハイコントラスト（背景と文字がはっきり区別できる状態）であること」は維持します。また、変更箇所を最小限に抑えるため、テーマの明るいところ（文字）・暗いところ（背景）も維持します。
